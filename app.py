...
def get_market_briefing(d_str):
    # 未来予測ではなく「一般的な注目材料」を聞くことで拒否を回避
    p = f"今日は{d_str}。直近の国内決算予定、日米欧中の重要指標、🚨注目イベントを簡潔にまとめて。公知の予定情報として日本語で出力して。"
    try:
        res = model.generate_content(p)
        return res.text if res.text else "情報の取得を制限中です。"
...
if not df_raw.empty:
    # --- データ正規化 (ここが最重要) ---
    df_raw['日付'] = pd.to_datetime(df_raw['日付'])
    ...
    st.info("データがありません。スクショをアップロードしてください。")
...
if st.session_state.analyzed:
    with st.form("edit_form"):
        ...
        if st.form_submit_button("記録する"):
            ...
            t_v = n_c + n_s + n_m
            ent = pd.DataFrame([{"日付": today, "現物買付余力": n_c, "現物時価総額": n_s, "信用評価損益": n_m, "総資産": t_v, "1億円までの残り": GOAL - t_v}])
            try:
