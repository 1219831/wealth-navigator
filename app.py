...
def get_market_briefing(date_key):
    p = f"今日は{date_key}。日本・海外の重要指標と決算、🚨重要イベントを簡潔にまとめて。"
    try:
        response = model.generate_content(p)
        return response.text if response.text else "情報の取得を制限中"
    except Exception as e:
        return f"💡 マーケット情報は準備中です。 (API状態: {str(e)[:25]})"
...
if not df_raw.empty:
    ...
    with st.expander("🗓️ 本日の投資イベント・ダイジェスト", expanded=True):
        ...
        st.write(get_market_briefing(t_key))
