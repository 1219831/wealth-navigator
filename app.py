import streamlit as st
from streamlit_gsheets import GSheetsConnection
import pandas as pd
from datetime import datetime as dt
from dateutil.relativedelta import relativedelta
import google.generativeai as gai
from PIL import Image
import plotly.graph_objects as go

# --- 1. 基本設定 ---
GOAL = 100000000
URL = "https://docs.google.com/spreadsheets/d/1-Elv0TZJb6dVwHoGCx0fQinN2B1KYPOwWt0aWJEa_Is/edit"
st.set_page_config(page_title="WealthNav PRO", layout="wide")

# --- 2. 司令部接続（API 404エラー対策済） ---
@st.cache_resource
def init_system():
    try:
        if "GEMINI_API_KEY" not in st.secrets: return None
        gai.configure(api_key=st.secrets["GEMINI_API_KEY"].strip())
        # 404エラー回避のため、プレフィックスなしの標準名称を指定
        return gai.GenerativeModel("gemini-1.5-flash")
    except: return None

def fetch_data():
    try:
        conn = st.connection("gsheets", type=GSheetsConnection)
        df = conn.read(spreadsheet=URL, ttl=0)
        if df is None or df.empty: return None
        # 日付変換
        df["日付"] = pd.to_datetime(df["日付"], errors='coerce')
        # 数値クレンジング（カンマ・¥・全角を強制排除）
        for c in ["総資産", "現物時価総額", "信用評価損益", "現物買付余力"]:
            if c in df.columns:
                df[c] = pd.to_numeric(df[c].astype(str).replace('[^0-9.-]', '', regex=True), errors='coerce').fillna(0)
        return df.dropna(subset=["日付"]).sort_values("日付").reset_index(drop=True)
    except: return None

model, df = init_system(), fetch_data()

# --- 3. メイン解析・表示 ---
if df is not None:
    # 有効な最新データを特定
    v_df = df[df["総資産"] > 0]
    if v_df.empty: v_df = df
    L = v_df.iloc[-1]
    T, M, S, C = int(L["総資産"]), int(L["信用評価損益"]), int(L["現物時価総額"]), int(L["現物買付余力"])
    now = dt.now()

    # 収支計算ロジック
    d_g = T - v_df.iloc[-2]["総資産"] if len(v_df) > 1 else 0
    m_s = v_df[v_df["日付"] >= now.replace(day=1, hour=0, minute=0, second=0)]
    m_g = T - m_s.iloc[0]["総資産"] if not m_s.empty else 0
    # 先月計算
    lm_s = (now.replace(day=1) - relativedelta(months=1))
    lm_e = now.replace(day=1, hour=0, minute=0, second=0)
    lm_df = df[(df["日付"] >= lm_s) & (df["日付"] < lm_e) & (df["総資産"] > 0)]
    p_gain = lm_df.iloc[-1]["総資産"] - lm_df.iloc[0]["総資産"] if not lm_df.empty else 0

    st.title("🚀 保有資産管理シート")
    
    # 4連メトリクス（総資産→今日→今月→先月）
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("💰 総資産合計", f"¥ {T:,}")
    with c1.expander("詳細内訳"):
        st.write(f"現物時価: ¥{S:,}\n信用損益: ¥{M:+,}\n買付余力: ¥{C:,}")
    c2.metric("📅 今日の収支", f"¥ {int(d_g):+,}")
    c3.metric("🗓️ 今月の収支", f"¥ {int(m_g):+,}")
    c4.metric("⏳ 先月の収支", f"¥ {int(p_gain):+,}")
    
    st.progress(max(0.0, min(float(T/GOAL), 1.0)), text=f"1億円達成まで残り ¥{(GOAL-T):,} ({T/GOAL:.2%})")

    # --- 4. 参謀本部：市場調査（本日 2026/03/06） ---
    st.divider()
    st.subheader("⚔️ 参謀本部：本日の市場調査報告")
    if model:
        p = f"""
        今日は2026年3月6日(金)です。プロの投資参謀として以下を【表形式】で報告せよ。
        1.【重要指標】本日発表の国内・海外の重要指標と時刻(今夜の米雇用統計 22:30等)
        2.【決算予定】本日発表の国内主要銘柄とコード。ない場合は「なし」
        3.【注意喚起】ボスの信用損益({M}円)を踏まえた本日のリスク管理助言を120字で。
        """
        try:
            with st.spinner("市場情報をリサーチ中..."):
                res = model.generate_content(p)
                st.markdown(res.text)
        except Exception as e:
            st.error(f"市場情報取得エラー: {e}")
    
    # --- 5. プロ仕様グラフ ---
    st.divider()
    def draw(data, k):
        data["MA5"] = data["総資産"].rolling(5, min_periods=1).mean()
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=data["日付"], y=data["総資産"], name="資産", fill='tozeroy', line=dict(color='#007BFF', width=3)))
        fig.add_trace(go.Scatter(x=data["日付"], y=data["MA5"], name="5日線", line=dict(color='#FFA500', dash='dot')))
        y_r = [data["総資産"].min()*0.95, data["総資産"].max()*1.05]
        fig.update_layout(template="plotly_dark", height=400, margin=dict(l=10, r=10, t=10, b=10), yaxis=dict(range=y_r, tickformat=",.0f"))
        st.plotly_chart(fig, use_container_width=True, key=k)

    tab = st.radio("グラフ表示:", ["日次推移", "週次推移", "月次推移"], horizontal=True)
    if "日次" in tab: draw(v_df, "d")
    elif "週次" in tab: draw(v_df.set_index("日付").resample("W").last().dropna().reset_index(), "w")
    else: draw(v_df.set_index("日付").resample("M").last().dropna().reset_index(), "m")

# --- 6. 更新セクション（画像アップロード 404対策済） ---
st.divider()
st.subheader("📸 スマホ更新：AIマルチ・スキャン")
# accept_multiple_files=True でカメラロールから3枚選択可能
ups = st.file_uploader("証券スクショを選択(最大3枚)", accept_multiple_files=True, type=['png', 'jpg', 'jpeg'])

if st.button("🚀 AI統合解析を実行", use_container_width=True, type="primary"):
    if ups and model:
        with st.spinner("AIが戦況を統合解析中..."):
            try:
                # 404エラー対策：リスト形式で渡す。画像の型を確実にPILにする
                # 指示：各項目の数値を個別に抽出・合算するよう命令
                analysis_input = ["松井証券の画像から、(1)現物時価総額 (2)現物買付余力 (3)信用評価損益 を数値で抽出してください。"]
                for f in ups[:3]:
                    img = Image.open(f).convert("RGB") # 型変換で安定化
                    analysis_input.append(img)
                
                # マルチモーダル生成の実行
                response = model.generate_content(analysis_input)
                st.success("解析完了")
                st.markdown("### 解析された資産データ")
                st.write(response.text)
                st.info("※この数値をスプレッドシートの最新行に反映してください。")
            except Exception as e:
                st.error(f"解析失敗（404対策済ですがエラーが発生しました）: {e}")
    else:
        st.warning("解析する画像をアップロードしてください。")
